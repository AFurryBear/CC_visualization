...
$(function() {
 ...
 // var myChart = echarts.init(document.getElementById('volume-viewer-display'), null, {renderer: 'canvas'});
  ...
  function loadVolumeData(id){
    ...
    //   }
    //   if (c.indexOf(name) == 0) {
    //     var id = decodeURI(c.substr(name.length, c.length-2));
      return "models/Group_result1/"+id+'_'+weightSelect+"_"+th+".json";
    }
  //   return "data.json";
  // }

  function setCookie(value_j) {
    // var minutes = 5; //此 cookie 将被保存 5 min
    // var exp　= new Date();
    // exp.setTime(exp.getTime() +minutes*60*1000);
    var templateName = $("[name=parcelSelect]:checked").val();
    var value_k;
    if (templateName==="HCPMMP"){
      if (value_j>180){
        value_k = PrefixInteger(value_j-180,3);
        value_j = PrefixInteger(value_j,3)
      }
      else{
        value_k= PrefixInteger(value_j,3)
        value_j = PrefixInteger(value_j+180,3);
      }
    }
    if (templateName==="BN_Atlas"){
      if (value_j%2 ==0){
        value_k = PrefixInteger(value_j,3);
        value_j = PrefixInteger(value_j+1,3);
      }
      else{
        value_j = PrefixInteger(value_j,3);
        value_k = PrefixInteger(value_j+1,3);
      }
    }
    //document.cookie=name +"="+ templateName+"_result/"+value_j + "_" + value_k + ";expires=" + exp.toGMTString();
    return templateName+"_result/"+value_j + "_" + value_k;
  }
  ...
  function getCookie(cname) {
    ...
    for(var i = 0; i <ca.length; i++) {
      ...
      if (c.indexOf(name) == 0) {
        var n=c.indexOf('+');
        var id_1 = decodeURI(c.substr(name.length, n-2));
        var id_2 = decodeURI(c.substr(n+1, c.length-n));


        return PrefixInteger(id_1,3)+"_"+PrefixInteger(id_2,3);
      }
    }
    return "models/S1200.HCPMMP.32K.label.gii";
  }
  ...
  function clearAllCookie() {
    ...
    for(var i = 0; i <ca.length; i++) {
      ...
      document.cookie = c[0] + '=0;expires=' + new Date(0).toUTCString()


    }
    location.href = "XYR_surf2CC.html";
  }
  ...
  window.viewer = BrainBrowser.SurfaceViewer.start("brainbrowser", function(viewer) {
        ...
        if (intensity_data) {
          if (event.ctrlKey) {
            ...
            if (fileName.substr(0,5) === 'S1200') {
              ...
              $.get(loadVolumeData(setCookie(intensity_data.values[pick_info.index]))).done(function (data) {
                // 填入数据
                myChart.setOption({
                  xAxis: {
                    data: data.x_dat
            ...
            //setCookie('v',intensity_data.values[pick_info.index]);
            // var max_value = model_data.intensity_data[0].max;
    ...
    $("#examples").click(function(e) {
      ...
      var examples = {
        pial: function() {
          ...
          viewer.loadModelFromURL("models/S1200.pial_MSMAll.32k_fs_LR.surf.gii", {
            format: "gifti",
            complete: function() {
              $("#vertex-data-wrapper").show();
              $("#pick-value-wrapper").show();

              viewer.loadIntensityDataFromURL("models/S1200.HCPMMP.32K.label.gii", {
                format: "gifti",
        ...
        inflated: function() {
          viewer.annotations.setMarkerRadius(1);
          viewer.loadModelFromURL("models/S1200.inflated_MSMAll.32k_fs_LR.surf.gii", {
            format: "gifti",
            complete: function() {
              $("#vertex-data-wrapper").show();
              $("#pick-value-wrapper").show();
        ...
        very_inflated: function() {
          viewer.annotations.setMarkerRadius(1);
          viewer.loadModelFromURL("models/S1200.very_inflated_MSMAll.32k_fs_LR.surf.gii", {
            format: "gifti",
            complete: function() {
              $("#vertex-data-wrapper").show();
              $("#pick-value-wrapper").show();
    ...
    //     },
    //     BN_Atlas: function() {
    //       viewer.loadIntensityDataFromURL("models/S1200.HCPMMP.32K.label.txt", {
    //         format: "txt",
    //         complete: hideLoading,
    //         cancel: defaultCancelOptions(current_request)
    //       });
    ...
    $("a.example[data-example-name=pial]").click();
